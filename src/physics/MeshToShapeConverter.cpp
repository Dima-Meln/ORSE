...
#include "physics/MeshToShapeConverter.h"

MeshToShapeConverter::MeshToShapeConverter()
{
  /*AnimatedMeshToShapeConverter am;
  am.addEntity(Entity);
  am.createTrimesh();*/
}
...
btBoxShape* VertexIndexToShape::createBox() {
  ...
  btBoxShape* shape(new btBoxShape(btVector3(sz.x,sz.y,sz.z)));
  return shape;
}
//------------------------------------------------------------------------------------------------
...
btBvhTriangleMeshShape* VertexIndexToShape::createTrimesh() {
  ...
  bool use32bitsIndices = false;
  btTriangleMesh* mTriMesh = new btTriangleMesh(use32bitsIndices);
  ...
  for(unsigned int n = 0; n < numFaces; ++n) {
    ...
    {
      const Vector3& vec = mVertexBuffer[*(mIndexBuffer + 1)];
      vertexPos[1][0] = vec.x;
      vertexPos[1][1] = vec.y;
      vertexPos[1][2] = vec.z;
    }
    {
      const Vector3& vec = mVertexBuffer[*(mIndexBuffer + 2)];
      vertexPos[2][0] = vec.x;
      vertexPos[2][1] = vec.y;
      vertexPos[2][2] = vec.z;
    }
