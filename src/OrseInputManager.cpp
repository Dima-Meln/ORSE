...
void InputManager::initialise( Ogre::RenderWindow *renderWindow ) {
    if( !mInputSystem ) {
        ...
        /*if (mInputSystem->getNumberOfDevices(OIS::MultiTouch) > 0) {
            mMultiTouch = static_cast<OIS::MultiTouch*>( mInputSystem->createInputObject( OIS::OISMultiTouch, true ) );
...
/*void InputManager::addMultiTouchListener( OIS::MultiTouchListener *MultiTouchListener, const std::string& instanceName ) {
    if( mMultiTouch ) {
        // Check for duplicate items
        itMultiTouchListener = mMultiTouchListeners.find( instanceName );
        if( itMultiTouchListener == mMultiTouchListeners.end() ) {
            mMultiTouchListeners[ instanceName ] = MultiTouchListener;
        }
        else {
            // Duplicate Item
        }
...
/*void InputManager::removeMultiTouchListener(const std::string& instanceName) {
  // Check if item exists
    itMultiTouchListener = mMultiTouchListeners.find( instanceName );
    if( itMultiTouchListener != mMultiTouchListeners.end() ) {
        mMultiTouchListeners.erase( itMultiTouchListener );
    }
    else {
        // Doesn't Exist
    }
...
/*void InputManager::removeMultiTouchListener(OIS::MultiTouchListener *MultiTouchListener) {
  itMultiTouchListener = mMultiTouchListeners.begin();
    itMultiTouchListenerEnd = mMultiTouchListeners.end();
    for(; itMultiTouchListener != itMultiTouchListenerEnd; ++itMultiTouchListener ) {
        if( itMultiTouchListener->second == MultiTouchListener ) {
            mMultiTouchListeners.erase( itMultiTouchListener );
            break;
        }
    }
...
/*void InputManager::removeAllMultiTouchListeners(void) {
  mMultiTouchListeners.clear();
}*/
 
void InputManager::setWindowExtents( int width, int height ) {
    /*const OIS::MultiTouchState &MultiTouchState = mMultiTouch->getMultiTouchStates().;
...
/*OIS::MultiTouch* InputManager::getMultiTouch() {
  return mMultiTouch;
}*/
...
/*bool touchMoved( const OIS::MultiTouchEvent &e ) {
  itMultiTouchListener    = mMultiTouchListeners.begin();
    itMultiTouchListenerEnd = mMultiTouchListeners.end();
    for(; itMultiTouchListener != itMultiTouchListenerEnd; ++itMouseListener ) {
        if(!itMultiTouchListener->second->touchMoved( e ))
            break;
    }
    return true;
}
bool touchPressed( const OIS::MultiTouchEvent &e ) {
  itMultiTouchListener    = mMultiTouchListeners.begin();
    itMultiTouchListenerEnd = mMultiTouchListeners.end();
    for(; itMultiTouchListener != itMultiTouchListenerEnd; ++itMouseListener ) {
        if(!itMultiTouchListener->second->touchPressed( e ))
            break;
    }
    return true;
}
bool touchReleased( const OIS::MultiTouchEvent &e ) {
  itMultiTouchListener    = mMultiTouchListeners.begin();
    itMultiTouchListenerEnd = mMultiTouchListeners.end();
    for(; itMultiTouchListener != itMultiTouchListenerEnd; ++itMouseListener ) {
        if(!itMultiTouchListener->second->touchReleased( e ))
            break;
    }
    return true;
}
bool touchCancelled( const OIS::MultiTouchEvent &e ) {
  itMultiTouchListener    = mMultiTouchListeners.begin();
    itMultiTouchListenerEnd = mMultiTouchListeners.end();
    for(; itMultiTouchListener != itMultiTouchListenerEnd; ++itMouseListener ) {
        if(!itMultiTouchListener->second->touchCanceled( e ))
            break;
    }
    return true;
}*/
